...
The Studio dashboard (http://localhost:54321) is protected by HTTP basic auth.
The login credentials are stored in `infra/supabase/.env`:

- **Username** — `DASHBOARD_USERNAME` (default `supabase`)
- **Password** — `DASHBOARD_PASSWORD`

Open that file to find the current values before signing in.
